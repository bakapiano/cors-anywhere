...
var BIND_HOST = process.env.BIND_HOST;
...
function withCORS(headers, request) {
  headers['access-control-allow-origin'] = request.headers['origin'] === undefined ? "*" : request.headers['origin'];
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  if (statusCode === 301 || statusCode === 302 || statusCode === 303 || statusCode === 307 || statusCode === 308) {
    ...
    if (parsedLocation) {
      ...
      // proxyRes.headers.location = requestState.proxyBaseUrl + url.parse(locationHeader).path;
      proxyRes.headers.location = `https://${url.parse(proxyRes.headers.location).hostname}.${BIND_HOST}${url.parse(locationHeader).path}`
    }
  }
  ...
  // console.log(requestState.proxyBaseUrl);
  ...
  if (proxyRes.headers['set-cookie']!==undefined) {
    proxyRes.headers['set-cookie'] = proxyRes.headers['set-cookie'].map((value) => {
      let containSameSite = false;
      let containSecure = false;
      let options = value.split(";").map((attribute) => {
        if (attribute.toLowerCase().trim().startsWith('domain=')) {
          let value = attribute.trim().replaceAll(/(d|D)omain=/g, '');
          return `Domain=${value}.${BIND_HOST}`;
        }
        if (attribute.toLowerCase().trim().startsWith('samesite')) {
          containSameSite = true;
          return 'SameSite=None';
        }
        if (attribute.toLowerCase().trim().startsWith('secure')) {
          containSecure = true;
        }
        return attribute.trim();
      });
      if (!containSameSite) options.push("SameSite=None");
      if (!containSecure)   options.push("Secure");
      console.log(options, containSameSite, containSecure, options.join(';'))
      return options.join(';');
    })
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var location = parseURL("https://" + req.headers.host.replace(`.${BIND_HOST}`, "") + "/" + req.url.slice(1));
