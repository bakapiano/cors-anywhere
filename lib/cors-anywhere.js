...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  if (statusCode === 301 || statusCode === 302 || statusCode === 303 || statusCode === 307 || statusCode === 308) {
    ...
    if (parsedLocation) {
      ...
      proxyRes.headers.location = `https://${url.parse(proxyRes.headers.location).hostname}.${host}${url.parse(locationHeader).path}`
    }
  }
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var location = parseURL("https://" + req.headers.host.replace(`.${host}`, "") + "/" + req.url.slice(1));
